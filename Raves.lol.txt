...
if AuthLua:Authenticate("Raves.lol") then
--[[
    ...
    if typeof(v) == "table" then
        ...
        if rawget(v, "Variables") and rawget(v, "Process") and typeof(KillFunc) == "function" and not Kill then
            ...
            local Old; Old = hookfunction(Kill, function(Info)
                if DEBUG then
                    warn(`Adonis Cheat tried to kill (fallback): {Info}`)
                end
            end)
...
local crosshair = loadstring(game:HttpGet("https://raw.githubusercontent.com/Shhflow/Shhshssufydhsjausbkq81twbsuz/refs/heads/main/Sigma"))()
...
TriggerBotFovCircle.Visible = Settings.Rage.TriggerBot.FOV.Show and Settings.Rage.TriggerBot.Enabled
TriggerBotFovCircle.Filled = false
TriggerBotFovCircle.Radius = Settings.Rage.TriggerBot.FOV.Size*3
...
local function LocateTheseNiggas()
    ...
    for _, player in ipairs(players:GetPlayers()) do
        if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            if not Settings.Rage.TriggerBot.TargetOnly or player == Script.Locals.Target and Script.Locals.IsTargetting then
...
local ZPred = tonumber(Settings.Rage.Prediction.Horizontal)
...
makemarker(placemarker, placemarker, Color3.fromRGB(255,209,220), 0.8, 0)
      ...
      local RootPart  = Character and Character:FindFirstChild('HumanoidRootPart')
        ...
        do
            Script.Functions.GetClosestPlayerNumbah = function()
                local Radius = Settings.Rage.AutoSelect.Enabled and Settings.Rage.Fov.Radius or math.huge
                local ClosestPlayer
            ...
            Script.Functions.GetTargetPredictedPosition = function()
                ...
                local Velocity = Settings.Rage.Resolver.Enabled and Vector3.new(Script.Locals.Resolver.ResolvedVelocity.X*Settings.Rage.Prediction.Horizontal, math.clamp(Script.Locals.Resolver.ResolvedVelocity.Y,-10,50)*Settings.Rage.Prediction.Vertical,Script.Locals.Resolver.ResolvedVelocity.Z*ZPred) or Vector3.new(Script.Locals.Target.Character.HumanoidRootPart.AssemblyLinearVelocity.X*Settings.Rage.Prediction.Horizontal,math.clamp(Script.Locals.Target.Character.HumanoidRootPart.AssemblyLinearVelocity.Y,-10,50)*Settings.Rage.Prediction.Vertical,Script.Locals.Target.Character.HumanoidRootPart.AssemblyLinearVelocity.Z*ZPred)
            ...
            Script.Functions.Resolve = function()
                if Settings.Rage.Enabled and Settings.Rage.Resolver.Enabled and Script.Locals.IsTargetting and Script.Locals.Target then
                    ...
                    Script.Locals.Resolver.ResolvedVelocity = lerp(Script.Locals.Resolver.ResolvedVelocity, RawVelocity, Settings.Rage.Resolver.Smoothness)
            ...
            Script.Functions.MouseAim = function()
                if Settings.Rage.Enabled and Settings.Rage.Camera and Script.Locals.IsTargetting and Script.Locals.Target then
                    local LalaPosition = (Script.Functions.GetTargetPredictedPosition().Position)
                    Smoothness = tonumber(Settings.Rage.Smoothing.Horizontal)
                    LookPosition = CFrame.new(CC.CFrame.p, LalaPosition)
                    CC.CFrame = CC.CFrame:Lerp(LookPosition, Smoothness, Enum.EasingStyle[Settings.Rage.EasingStyle], Enum.EasingDirection[Settings.Rage.EasingDirection])
                end
            end

            Script.Functions.UpdateFieldOfView = function()
                Script.Utility.Drawings["FieldOfViewVisualizer"].Visible = Settings.Rage.Enabled and Settings.Rage.AutoSelect.Enabled and Settings.Rage.Fov.Visualize.Enabled
            ...
            if Settings.Rage.Enabled and Settings.Rage.Visuals.Enabled and Script.Locals.IsTargetting and Script.Locals.Target and Settings.Rage.Visuals.Dot.Enabled then
            ...
            Script.Functions.GunEvents = function()
                ...
                if CurrentGun and CurrentGun.IsGunEquipped and CurrentGun.Tool then
                    ...
                    if not Script.Connections.GunConnections["GunActivated"] and Settings.Rage.Enabled and Settings.Rage.Silent and Script.Locals.AimViewer.MouseRemoteFound then
                        Script.Connections.GunConnections["GunActivated"] = Script.Functions.Connection(CurrentGun.Tool.Activated, function()
                            if Script.Locals.IsTargetting and Script.Locals.Target then
                                if Settings.Rage.AimViewer then
                                    local Arguments = Script.Locals.AimViewer.MouseRemoteArgs

                                    Arguments[Script.Locals.AimViewer.MouseRemotePositionIndex] = (Script.Functions.GetTargetPredictedPosition().Position)
                                    Script.Locals.AimViewer.MouseRemote:FireServer(unpack(Arguments))
                                end
                            end
                        end)
                    end
            ...
            Script.Functions.Air = function()
                if Settings.Rage.Enabled and Script.Locals.IsTargetting and Script.Locals.Target and Settings.Rage.Air.Enabled then
                    ...
                    if Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
                        if Settings.Rage.BetaAirshot then
                        ZPred = 0
                    end
                        Script.Locals.JumpOffset = Settings.Rage.Air.JumpOffset.Offset
                    else
                        ZPred = Settings.Rage.Prediction.Horizontal
            ...
            Script.Functions.VelocitySpoof = function()
                local ShowVisualizerDot = Settings.Aim.VelocitySpoofer.Enabled and Settings.Aim.VelocitySpoofer.Visualize.Enabled

                Script.Utility.Drawings["VelocityDot"].Visible = ShowVisualizerDot


                if Settings.Aim.VelocitySpoofer.Enabled then
                    --// Variables
                    local Type = Settings.Aim.VelocitySpoofer.Type
                    local HumanoidRootPart = LocalPlayer.Character.HumanoidRootPart
                    local Velocity = HumanoidRootPart.Velocity

                    --// Main
                    if Type == "Underground" then
                        HumanoidRootPart.Velocity = HumanoidRootPart.Velocity + Vector3.new(0, -Settings.Aim.VelocitySpoofer.Yaw, 0)
                    elseif Type == "Sky" then
                        HumanoidRootPart.Velocity = HumanoidRootPart.Velocity + Vector3.new(0, Settings.Aim.VelocitySpoofer.Yaw, 0)
                    elseif Type == "Multiplier" then
                        HumanoidRootPart.Velocity = HumanoidRootPart.Velocity + Vector3.new(Settings.Aim.VelocitySpoofer.Yaw, Settings.Aim.VelocitySpoofer.Pitch, Settings.Aim.VelocitySpoofer.Roll)
                    elseif Type == "Custom" then
                        HumanoidRootPart.Velocity = Vector3.new(Settings.Aim.VelocitySpoofer.Yaw, Settings.Aim.VelocitySpoofer.Pitch, Settings.Aim.VelocitySpoofer.Roll)
                    elseif Type == "Prediction Breaker" then
                        HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
                    end

                    if ShowVisualizerDot then
                        local ScreenPosition = Script.Functions.WorldToScreen(LocalPlayer.Character.HumanoidRootPart.Position + LocalPlayer.Character.HumanoidRootPart.Velocity * Settings.Aim.VelocitySpoofer.Visualize.Prediction)

                        Script.Utility.Drawings["VelocityDot"].Position = ScreenPosition.Position
                        Script.Utility.Drawings["VelocityDot"].Color = Settings.Aim.VelocitySpoofer.Visualize.Color
                    end
            ...
            Script.Functions.CSync = function()
                Script.Utility.Drawings["CFrameVisualize"].Parent = Settings.Aim.CSync.Visualize.Enabled and Settings.Aim.CSync.Enabled and Script.AuraIgnoreFolder or nil

                if Settings.Aim.CSync.Enabled then
                    local Type = Settings.Aim.CSync.Type
                    ...
                    if Type == "Target Strafe" and Script.Locals.IsTargetting and Script.Locals.Target and Settings.Rage.Enabled then
                        local CurrentTime = tick()
                        FakeCFrame = CFrame.new(Script.Locals.Target.Character.HumanoidRootPart.Position) * CFrame.Angles(0, 2 * math.pi * CurrentTime * Settings.Aim.CSync.TargetStrafe.Speed % (2 * math.pi), 0) * CFrame.new(0, Settings.Aim.CSync.TargetStrafe.Height, Settings.Aim.CSync.TargetStrafe.Distance)
                    elseif Type == "Random Target" and Script.Locals.IsTargetting and Script.Locals.Target and Settings.Rage.Enabled then
                        FakeCFrame = CFrame.new(Script.Locals.Target.Character.HumanoidRootPart.Position + Vector3.new(math.random(-Settings.Aim.CSync.RandomDistance, Settings.Aim.CSync.RandomDistance), math.random(-0, Settings.Aim.CSync.RandomDistance), math.random(-Settings.Aim.CSync.RandomDistance, Settings.Aim.CSync.RandomDistance))) * CFrame.Angles(math.rad(math.random(0, 360)), math.rad(math.random(0, 360)), math.rad(math.random(0, 360)))
                    end
                    ...
                    for _, Part in pairs(Script.Utility.Drawings["CFrameVisualize"]:GetChildren()) do
                        Part.Color = Settings.Aim.CSync.Visualize.Color
                    end
                    LocalPlayer.Character.HumanoidRootPart.CFrame = FakeCFrame
                    RunService.RenderStepped:Wait()
                    if Settings.Aim.CSync.Spoof  then
            ...
            Script.Functions.Network = function()
                if Settings.Aim.Network.Enabled then
                    if (tick() - Script.Locals.NetworkPreviousTick) >= ((Settings.Aim.Network.Amount / math.pi) / 10000) or (Settings.Aim.Network.WalkingCheck and LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0) then
            ...
            Script.Functions.VelocityDesync = function()
                if Settings.Aim.VelocityDesync.Enabled then
                    local HumanoidRootPart = LocalPlayer.Character.HumanoidRootPart
                    local Velocity = HumanoidRootPart.Velocity
                    local Amount = Settings.Aim.VelocityDesync.Range * 1000
            ...
            Script.Functions.DaCoolBoyDesync = function()
                if Settings.Aim.DaCoolBoyDesync then
            ...
            Script.Functions.FFlagDesync = function()
                if Settings.Aim.FFlagDesync.Enabled then
                    for FFlag, _ in pairs(Settings.Aim.FFlagDesync.FFlags) do
                        local Value = Settings.Aim.FFlagDesync.Amount
                        setfflag(FFlag, tostring(Value))

                        RunService.RenderStepped:Wait()
                        if Settings.Aim.FFlagDesync.SetNew then
                            setfflag(FFlag, Settings.Aim.FFlagDesync.SetNewAmount)
                        end
                    end
                end
            end
            ...
            Script.Functions.UpdateLookAt = function()
                if Settings.Rage.Enabled and Settings.Rage.Silent and Settings.Rage.LookAt and Script.Locals.IsTargetting and Script.Locals.Target and LocalPlayer then
            ...
            Script.Functions.UpdateSpectate = function()
                if Settings.Rage.Enabled and Settings.Rage.Silent and Settings.Rage.Spectate and Script.Locals.IsTargetting and Script.Locals.Target then
    ...
    do
        ...
        Script.Utility.Drawings["VelocityDot"] = Script.Functions.CreateDrawing("Circle", {
            Visible = false,
            Color = Settings.Aim.VelocitySpoofer.Visualize.Color,
            Radius = 6,
            Filled = true
        })
...
Part.Parent = workspace
    ...
    do
        ...
        __index = hookmetamethod(game, "__index", newcclosure(function(Self, Index)
            if not checkcaller() and Settings.Aim.CSync.Enabled and Settings.Aim.CSync.Spoof and Script.Locals.SavedCFrame and Index == "CFrame" and Self == LocalPlayer.Character.HumanoidRootPart then
            ...
            if not checkcaller() and Method == "FireServer" then
                for _, Argument in pairs(Arguments) do
                    if typeof(Argument) == "Vector3" then
                        Script.Locals.AimViewer.MouseRemote = Self
                        Script.Locals.AimViewer.MouseRemoteFound = true
                        WOAHHH = Arguments
                        Script.Locals.AimViewer.MouseRemoteArgs = Arguments
                        Script.Locals.AimViewer.MouseRemotePositionIndex = _
                        NEINIGGANEINEI = _

                        if Settings.Rage.Enabled and Settings.Rage.Silent and not Settings.Rage.AimViewer and Script.Locals.IsTargetting and Script.Locals.Target and not Settings.Rage.UseIndex then
...
Sexy[1] = hookmetamethod(ClientThinf:GetMouse(), "__index", newcclosure(function(self, index)
    if index == "Hit" and Settings.Rage.Enabled and Settings.Rage.Silent and not Settings.Rage.AimViewer and Script.Locals.IsTargetting and Script.Locals.Target and Settings.Rage.UseIndex then
...
do
    local window = library:Window({Name = [[Raves <font color="rgb(255,209,220)">.lua</font>]], Size = Vector2.new(450,450)});
...
Outline.MouseButton1Click:Connect(function()
    ...
    do -- tabs
        local aiming = window:Page({Name = "Rage"});
        local looking = window:Page({Name = "Visuals"});
        local skibiding = window:Page({Name = ""});
            ...
            do -- silent elements
                ...
                Tobol.Activated:Connect(function()
                    ...
                    Script.Locals.Target = Script.Locals.IsTargetting and NewTarget.Character and NewTarget or nil
                        ...
                        if Settings.Rage.Alerts and Script.Locals.Target ~= nil then
...
draggable(Fraame)
                ...
                main:List({
                    ...
                    Callback = function(v)
                        Settings.Rage.AimPart = v
                    end
                })

                main:Textbox({Name = "Horizontal", Default = "0.14", Callback = function(v)
                    Settings.Rage.Prediction.Horizontal = tonumber(v)
                    end
                })

                main:Textbox({Name = "Vertical", Default = "0.12", Callback = function(v)
                    Settings.Rage.Prediction.Vertical = tonumber(v)
                    end
                })

                main:Toggle({Name = "Auto Pred", Callback = function(v)
                    Settings.Rage.PingBased = v
                end})
RunService.Stepped:connect(function()
if Settings.Rage.PingBased then
    pingvalue = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
split = string.split(pingvalue,'(')
ping = tonumber(split[1])
    Settings.Rage.Prediction.Vertical = tonumber(ping/1500)
    if ping <200 then
        Settings.Rage.Prediction.Horizontal = 0.2198343243234332
    elseif ping < 170 then
        Settings.Rage.Prediction.Horizontal = 0.2165713
    elseif ping < 160 then
        Settings.Rage.Prediction.Horizontal = 0.16242
    elseif ping < 150 then
        Settings.Rage.Prediction.Horizontal = 0.158041
    elseif ping < 140 then
        Settings.Rage.Prediction.Horizontal = 0.155313
    elseif ping < 130 then
        Settings.Rage.Prediction.Horizontal = 0.152692
    elseif ping < 120 then
        Settings.Rage.Prediction.Horizontal = 0.153017
    elseif ping < 110 then
        Settings.Rage.Prediction.Horizontal = 0.15165
    elseif ping < 100 then
        Settings.Rage.Prediction.Horizontal = 0.1483987
    elseif ping < 80 then
        Settings.Rage.Prediction.Horizontal = 0.1451340
    elseif ping < 70 then
        Settings.Rage.Prediction.Horizontal = 0.143633
    elseif ping < 65 then
        Settings.Rage.Prediction.Horizontal = 0.1374236
    elseif ping < 50 then
        Settings.Rage.Prediction.Horizontal = 0.13644
    elseif ping < 30 then
        Settings.Rage.Prediction.Horizontal = 0.12452476
    end
end
end)
            end
            ...
            do -- Target
                ...
                trgt:Toggle({Name = "Look At", Callback = function(v)
                    Settings.Rage.LookAt = v
                end})

                trgt:Toggle({Name = "View At", Callback = function(v)
                    Settings.Rage.Spectate = v
                end})

                trgt:Toggle({Name = " Aim Viewer", Callback = function(v)
                    Settings.Rage.AimViewer = v
                end})

                trgt:Toggle({Name = "Mouse.Hit Index", Callback = function(v)
                    Settings.Rage.UseIndex = v
                end})
            ...
            do -- Cam
                ...
                cam:List({
                    ...
                    Default = "In",
                    Callback = function(v)
                        Settings.Rage.EasingDirection = v
                    end
                })

                cam:Textbox({Name = "Smoothing", Default = "1", Callback = function(v)
                    Settings.Rage.Smoothing.Horizontal = v
                    end
                })
        ...
        do
           ...
           do
                rslvr:Toggle({Name = "Enabled", Callback = function(v)
                    Settings.Rage.Resolver.Enabled = v
                end})

                rslvr:Textbox({Name = "Smoothness", Default = "0.5", Callback = function(v)
                    Settings.Rage.Resolver.Smoothness = tonumber(v)
                    end
                })
           end
       end
        ...
        local strafearea = aiming:Section({Name = "Strafe", Side = "Right", Size = 245})
            do
                ...
                strafearea:Toggle({Name = "Enabled", Callback = function(v)
                    Settings.Aim.CSync.Enabled = v
                end})

                strafearea:Toggle({Name = "Spoof", Callback = function(v)
                    Settings.Aim.CSync.Spoof = v
                    Settings.Aim.CSync.Visualize.Enabled = v
                end})

                strafearea:Toggle({Name = "Randomized", Callback = function(v)
                        if v then
                                Settings.Aim.CSync.Type = "Random Target"
                            else
                                Settings.Aim.CSync.Type = "Target Strafe"
                            end
                end})

                strafearea:Textbox({Name = "Randomized Amount", Default = "1", Callback = function(v)
                    Settings.Aim.CSync.RandomDistance = tonumber(v)
                    end
                })

                strafearea:Textbox({Name = "Speed", Default = "1", Callback = function(v)
                    Settings.Aim.CSync.TargetStrafe.Speed = tonumber(v)
                    end
                })

                strafearea:Textbox({Name = "Height", Default = "1", Callback = function(v)
                    Settings.Aim.CSync.TargetStrafe.Height = tonumber(v)
                    end
                })

                strafearea:Textbox({Name = "Distance", Default = "1", Callback = function(v)
                    Settings.Aim.CSync.TargetStrafe.Distance = tonumber(v)
                    end
                })
        ...
        local trgtstff = looking:Section({Name = "Target Visuals", Side = "Right", Size = 105})
            do
                trgtstff:Toggle({Name = "Dot", Callback = function(v)
                    Settings.Rage.Visuals.Dot.Enabled = v
                end})
                trgtstff:Toggle({Name = "Cham", Callback = function(v)
                    Settings.Rage.Visuals.Chams.Enabled = v
                end})
                trgtstff:Toggle({Name = "Tracer", Callback = function(v)
                     Settings.Rage.Visuals.Tracer.Enabled = v
                end})
            end
        end
        do
        local ESPstff = looking:Section({Name = "ESP", Side = "Right", Size = 305})
            do
                ESPstff:Toggle({Name = "Enable", Callback = function(v)
